 ...
 * of that are traps, and the site had the second: every page set its own
 * `title`, `description` and canonical, and every page still emitted the home
 * page's `og:url`, `og:title` and `og:description`. Measured on the export
 * before this helper -- all eleven content pages plus the 404 advertised
 * `og:url` as the site root, so a share of the privacy policy on LinkedIn or
 * Slack unfurled as the home page.
 *
