...
/**
 * Where ContentImage actually puts a given file, asked of the component itself
 * rather than restated here. A second copy of the directory would be one more
 * thing that can drift from the thing it describes — which is the shape of the
 * bug this file exists to catch.
 */
function contentImageRef(name: string): string {
  const markup = renderToStaticMarkup(<ContentImage name={name} alt="" width={1} height={1} />)
  const src = markup.match(/src="([^"]+)"/)?.[1]
  if (!src) throw new Error('ContentImage rendered no src — update this helper.')
  return src
}
...
function referencedAssets(): { ref: string; file: string }[] {
  ...
  const walk = (dir: string) => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      // `ContentImage` takes a file NAME and builds the path itself, so its
      // images carry no literal `src="/..."` at the call site. Without this they
